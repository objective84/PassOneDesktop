...
namespace PassOne.Business
{
    public class CredentialsManager : ManagerBase
    {
        ...
        public int CreateCredentials(Credentials creds)
        {
        ...
        public Credentials FindCredentials(int id)
        {
            try
            {
                var creds =
                    (GetService(Services.CredentialsData) as IPassOneDataSvc).RetreiveById(id) as Credentials;
                return creds;
            }
            catch (CryptographicException)
            {
